...
Die Music-Quiz-Web-App ermöglicht es jedem Nutzer, Quizze zu nischigen Musikrichtungen zu erstellen und zu veröffentlichen. Dabei entsteht jedoch eine wichtige UX-Herausforderung bei der Klassifizierung der Genres während der Quiz-Erstellung. 
Die App soll das Alleinstellungsmerkmal haben, dass keine Musik-Quiz-App Quizze für spezifische Communitys und Genres abdeckt. Eine Liste mit den klassischsten Genres reicht daher nicht aus, um diesem Merkmal gerecht zu werden. Ein freies Textfeld, in dem User das Genre einfach reinschreiben können, würde zu Strukturlosigkeit und Inkonsistenz führen. 
...
Zum einen sind die Hauptgenres aufgelistet Pro Zeile ist ein Hauptgenre. Darauf folgen innerhalb der Klammer sogenannte Subgenres, die das Hauptgenre spezifischer abgrenzen. 
Subgenres werden per Texteingabe mit Autocomplete-Vorschlägen aus bereits verwendeten Subgenres eingetragen. Nutzer können auch völlig neue Subgenres frei eingeben. Werden Subgenres häufig benutzt, werden sie manuell in die genres.json-Datei hinzugefügt.
Die Entscheidung wurde im ganzen Team getroffen und wurde von Herrn Ecks Vorschlag aus den Peer-Reviews beeinflusst.

## Regarded Options

- Hauptgenres bilden stabile Ankerpunkte für die Filterung und das Browsing.
